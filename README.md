...
Lighthouse can be configured to run Progressive Web App, SEO, and Best Practice category ests. See the config file in `config/simorgh/simorgh.js`. Optionally, thresholds can be passed to indicate a minimum score that must be met in each category. `a11y-tests-web` will generate a summary report in JSON, saved to `a11y-tests-web/lighthouse-report.json`. An OUTPUT_JSON path can also be specified.
