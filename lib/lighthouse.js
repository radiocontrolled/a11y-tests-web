...
function run() {
  ...
  const thresholds = lighthouseConfig.config.thresholds.minimumScores || null;
  ...
  return executeSequentially(tasks)
    .then(() => {
      ...
      const result = parseScores(jsonOutput, thresholds);
      console.log('the parseScores result is:', result);
      // if (!parseScores(jsonOutput, thresholds)) {
      //   logger.error('Does not meet threshold');
      //   process.exit(1);
      // }
    });
}
...
function parseScores(jsonOutput, thresholds) {
  jsonOutput.urls.forEach((url) => {
    url.results.forEach((result) => {
      console.log('result', result);
      thresholds.forEach((threshold) => {
        console.log('threshold', threshold);
        if (result.name === threshold.name) {
          if (result.score <= threshold.score) {
            console.log('I have not met a threshold');
            console.log('url', url);
  ...
  console.log('I have met a threshold');
  return true;
}
