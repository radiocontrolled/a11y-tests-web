...
function run() {
  ...
  const thresholds = getThresholdConfig(lighthouseConfig);
  ...
  return executeSequentially(tasks)
    .then(() => {
      ...
      if (thresholds) outputFailingThresholdResults(jsonOutput, thresholds);
    });
}

// rename
function outputFailingThresholdResults(jsonOutput, thresholds) {
  const results = getFailingCategoriesFromJsonOutput(jsonOutput.urls, thresholds);

  if (results) {
    results.forEach((result) => {
      logger.error(`Lighthouse threshold not met on ${result.url}: Category '${result.category}' has pass threshold of ${result.passThreshold} but scored ${result.score}`);
    });
    process.exit(1);
  }
}
...
function getFailingCategoriesFromJsonOutput(jsonOutput, thresholds) {
  const failingTests = [];
  jsonOutput.forEach((endpoint) => {
    endpoint.results.forEach((result) => {
      thresholds.minimumScores.forEach((threshold) => {
        if (threshold.name === result.name && threshold.score > result.score) {
          failingTests.push({
            url: endpoint.url,
            category: threshold.name,
            passThreshold: threshold.score,
            score: result.score
          });
        }
      });
  ...
  return failingTests.length > 1 ? failingTests : false;
}
...
function getThresholdConfig(lighthouseConfig) {
  return lighthouseConfig.config.hasOwnProperty('thresholds') ? lighthouseConfig.config.thresholds : null;
}
